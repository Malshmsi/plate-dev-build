...
export class PlatesCatalogComponent implements OnInit {
  ...
  plates: numberPlate[] = [
    {
      ...
      style: 'ADOLD',
      price: 1234.21,
      compareAtPrice: 1243.65,
      phoneNumber: '0553343331',
      discounted: false,
    },
    {
      id: 2,
  ...
  updateEmiratesModel(event: any) {
    if (event.target.checked) {
      ...
      let findEmirate = this.selectedEmirates.findIndex(
  ...
  updateStylesModel(event: any) {
    if (event.target.checked) {
      ...
      let findStyle = this.selectedStyles.findIndex(
        (e: any) => e.style == event.target.name
      );
  ...
  updateDigitsModel(event: any) {
    if (event.target.checked) {
      let findDigit = this.digitsList.find(
        (e) => parseInt(e.digit) == event.target.name.length
      );
      this.selectedDigits.push(findDigit);
    } else {
      let findDigit = this.selectedDigits.findIndex(
        (e: any) => parseInt(e.digit) == event.target.name.length
      );
  ...
  updateCodesModel(event: any) {
    if (event.target.checked) {
      ...
      let findCode = this.selectedCodes.findIndex(
        (e: any) => e.code == event.target.name
      );
  ...
  updateFilteredPlates() {
    ...
    this.numberSearchedList.length > 0
      ? (searchedNumbers = [...this.numberSearchedList])
      : (searchedNumbers = [...this.numberSearchedList]);

    let emiratesSelected: any[] = [];
    this.selectedEmirates.length > 0
      ? (emiratesSelected = [...this.selectedEmirates])
      : (emiratesSelected = [...this.emiratesList]);

    let stylesSelected: any[] = [];
    this.selectedStyles.length > 0
      ? (stylesSelected = [...this.selectedStyles])
      : (stylesSelected = [...this.stylesList]);

    let codesSelected: any[] = [];
    this.selectedCodes.length > 0
      ? (codesSelected = [...this.selectedCodes])
      : (codesSelected = [...this.codesList]);

    let digitsSelected: any[] = [];
    this.selectedDigits.length > 0
      ? (digitsSelected = [...this.selectedDigits])
      : (digitsSelected = [...this.digitsList]);

    //filter filter results
    searchedNumbers.length == 0
      ? (this.filteredPlates = this.plates.filter((plate) => {
          return (
            emiratesSelected.includes(plate.emirate) &&
            stylesSelected.includes(plate.style)
          );
        }))
      : (this.filteredPlates = this.plates.filter((plate) => {
          return (
            searchedNumbers.includes(plate.plateNumber) &&
            emiratesSelected.includes(plate.emirate) &&
            stylesSelected.includes(plate.style) &&
            codesSelected.includes(digitsSelected)
          );
        }));
  ...
  ngOnInit() {
    ...
    //set digits list
    this.digitsList = [{ digit: 2 }, { digit: 3 }, { digit: 4 }, { digit: 5 }];
